...
enum BusOp { BUS_RD, BUS_RDX, BUS_UPDATE };
...
public:
    Cache(int s, int e, int b) : num_sets(1 << s), assoc(e), block_size(1 << b),
        set_index_bits(s), block_offset_bits(b), miss_count(0), access_count(0),
        eviction_count(0), writeback_count(0) {
...
public:
    Core(const std::string& trace_filename, int s, int e, int b) : instruction_count(0),
        total_cycles(0), idle_cycles(0), blocked(false), block_cycles_remaining(0) {
    ...
    double get_miss_rate() const {
        return cache->get_access_count() > 0 ?
            static_cast<double>(cache->get_miss_count()) / cache->get_access_count() : 0.0;
    }
